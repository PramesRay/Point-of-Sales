      ...
      <div v-if="!props.loading">
        <v-card class="bg-lightsecondary mt-5">
          <div v-if="latestTransaction" class="pa-5">
            <span class="text-subtitle-2 text-medium-emphasis text-disabled">{{ formatDate(latestTransaction?.date).slice(0, -11) }} : {{ formatDate(latestTransaction?.date).slice(-5) }}</span>
          ...
          <perfect-scrollbar :style="{ maxHeight: mdAndUp? '25rem' : '12rem'}">
            <v-list v-if="listTransaction.length > 0"  class="py-0">
              <v-list-item v-for="(listTransaction, i) in listTransaction" :key="i" :value="listTransaction" color="secondary" rounded="sm">
                <span class="text-subtitle-2 text-medium-emphasis text-disabled">{{ formatDate(listTransaction?.date).slice(0, -11) }} : {{ formatDate(listTransaction?.date).slice(-5) }}</span>
                <div class="d-inline-flex align-center justify-space-between w-100">
                  <div>
